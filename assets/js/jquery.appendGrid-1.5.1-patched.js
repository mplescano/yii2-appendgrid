        ...
        for (var z = 0; z < numOfRow; z++) {
                        ...
                        if (settings.columns[y].ctrlOptions.length > 0) {
                            if ($.isPlainObject(settings.columns[y].ctrlOptions[0])) {
                                ...
                                for (var x = 0; x < settings.columns[y].ctrlOptions.length; x++) {
                                    ...
                                    if ($.isArray(settings.columns[y].ctrlOptions[x].attributes)) {
                                        if (settings.columns[y].ctrlOptions[x].attributes.length > 0) {
                                            if ($.isPlainObject(settings.columns[y].ctrlOptions[x].attributes[0])) {
                                                for (var attrX = 0; attrX < settings.columns[y].ctrlOptions[x].attributes.length; attrX++) {
                                                    option.attr(settings.columns[y].ctrlOptions[x].attributes[attrX].attrName, settings.columns[y].ctrlOptions[x].attributes[attrX].attrValue);
                                                }
                                            }
                                        }
                                    }
            ...
            if (!settings._hideLastColumn || settings.columns.length > settings._visibleCount) {
                ...
                for (var y = 0; y < hidden.length; y++) {
                    ...
                    if ($.isFunction(settings.nameFormatter)) {
                    	ctrl.name = settings.nameFormatter(settings.idPrefix, settings.columns[hidden[y]].name, uniqueIndex);
                    } else {
                        ctrl.name = ctrl.id;
                    }
                    ctrl.type = 'hidden';
